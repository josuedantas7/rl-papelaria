    ...
    <div>
        ...
        <div className='mx-20 max-[700px]:mx-4 rounded-lg p-3 mt-6 bg-zinc-200'>
          <div className='w-full flex border-2 font-bold'>
            <p className='text-start w-2/5'>Nome</p>
            <p className='text-center w-1/5'>Imagem</p>
            <p className='text-center w-1/5'>Editar</p>
            <p className='text-end w-1/5'>Excluir</p>
          </div>
          <div className='flex flex-col gap-3'>
            {categories.map((category) => (
              <div key={category.id} className='w-full py-2 px-2 rounded-lg flex border-2 border-gray-500 items-center'>
                <p className='text-start w-2/5'>{category.name}</p>
                <Image className='mx-auto' src={category.image} height={80} width={80} alt='Imagem da categoria' />
